...
import { LogOut, LayoutDashboard, Send, Server, Power, Activity, TerminalSquare, ShieldAlert } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const navItems = [
    ...
    { name: "Terminal", icon: TerminalSquare, active: false },
  ];
          ...
          {navItems.map((item) => (
            <div 
              ...
              className={clsx(
                ...
                item.active ? "bg-white/5 text-white" : "text-zinc-500 hover:text-white hover:bg-white/5"
              )}>
              {item.active && <motion.div layoutId="navIndicator" className="absolute left-0 top-1/4 bottom-1/4 w-1 bg-red-600 rounded-r" />}
              <item.icon className={clsx("w-5 h-5 transition-colors", item.active ? "text-red-500" : "group-hover:text-red-400")} />
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-24 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-10 shrink-0 relative z-10">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-inter uppercase tracking-widest mt-1">Live Telemetry Data</p>
          </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
              ...
              <div className="xl:col-span-5 flex flex-col">
                ...
                <div className="bg-[#0a0a0a] border border-white/5 p-8 flex flex-col flex-1 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-red-600/5 blur-[40px] pointer-events-none" />
